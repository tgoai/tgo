"""AgentBay Desktop (Cloud Desktop) Controller."""
...
class DesktopController(AgentBayController):
    """Controller for AgentBay ComputerUse (cloud desktop) environment.
    ...
    async def create_session(
        ...
        """Create a new AgentBay desktop session.

        Args:
            environment_type: Should be "desktop" for this controller
            image_id: Optional image ID (e.g., "windows_latest", "linux_latest")
        ...
        try:
            ...
            if not image_id:
                raise ValueError(
                    ...
                    "请在 AgentBay 控制台创建云桌面镜像，并在平台配置中填写镜像 ID。\n"
            ...
            if result is None or result.session is None:
                image_hint = f" (当前配置: {image_id or 'windows_latest'})" if image_id else ""
                raise ValueError(
            ...
            logger.info(f"Created desktop session: {session_id}")
            return session_id
            ...
            logger.error(f"Failed to create desktop session: {error_msg}")
    ...
    async def take_screenshot(self, session_id: str) -> bytes:
        ...
        try:
            # Use AgentBay's screenshot capability for desktop
    ...
    async def click(self, session_id: str, x: int, y: int) -> bool:
        ...
        try:
            result = session.computer.click(x, y)
            if result.success:
    ...
    async def type_text(self, session_id: str, text: str) -> bool:
        ...
        try:
            result = session.computer.type_text(text)
            if result.success:
    ...
    async def swipe(
        ...
        On desktop, this is implemented as a mouse drag operation.
        ...
        try:
            # Desktop equivalent: mouse drag
            result = session.computer.drag(start_x, start_y, end_x, end_y)
            if result.success:
                logger.debug(f"Dragged from ({start_x}, {start_y}) to ({end_x}, {end_y})")
                return True
            else:
                logger.error(f"Failed to drag: {result.error_message}")
                return False
        except Exception as e:
    ...
    async def press_back(self, session_id: str) -> bool:
        """Press the back key (Escape on desktop).
        ...
        try:
            result = session.computer.press_key("Escape")
            if result.success:
                logger.debug("Pressed Escape key")
                return True
            else:
                logger.error(f"Failed to press Escape: {result.error_message}")
                return False
        except Exception as e:
    ...
    async def launch_app(self, session_id: str, package_name: str) -> bool:
        ...
        On desktop, this starts the application by name/path.

        Args:
            session_id: Session ID
            package_name: Application name or path (e.g., "WeChat.exe")
        ...
        try:
            # For desktop, we use the computer module to start apps
            result = session.computer.start_app(start_cmd=package_name)
            if result.success:
    ...
    async def get_installed_apps(self, session_id: str) -> list[str]:
        """Get list of installed application commands.

        Args:
            session_id: Session ID

        Returns:
            List of start commands
        """
        session = await self._ensure_session(session_id)
    ...
    async def press_key(self, session_id: str, key: str) -> bool:
        ...
        try:
            result = session.computer.press_key(key)
            if result.success:
                logger.debug(f"Pressed key: {key}")
                return True
            else:
                logger.error(f"Failed to press key {key}: {result.error_message}")
                return False
        except Exception as e:
